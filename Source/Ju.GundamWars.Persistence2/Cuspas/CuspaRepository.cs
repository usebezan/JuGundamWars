...
public class CuspaRepository(IDbContextFactory<GwDbContext> factory, ILogger<CuspaRepository> logger)
    : RepositoryBase2<Cuspa>(factory, logger), ICuspaRepository
{

    public override Cuspa? SelectById(int id) =>
        Execute(dbContext => dbContext.IncludedCuspas.FirstOrDefault(e => e.Id == id));

    public override List<Cuspa> SelectAll() =>
        Execute(dbContext => dbContext.IncludedCuspas.OrderBy(e => e.Name).ThenBy(e => e.Id).ToList());
