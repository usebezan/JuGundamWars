...
public partial class CoMobileStatus : ModelBase
{
    ...
    public CoMobileStatus AddWithBonus(CoMobileStatus status, decimal value)
    {
        Hp += status.Hp.Multiply(value);
        BeamAttack += status.BeamAttack.Multiply(value);
        PhysicalAttack += status.PhysicalAttack.Multiply(value);
        BeamDefence += status.BeamDefence.Multiply(value);
        PhysicalDefence += status.PhysicalDefence.Multiply(value);
        CriticalDamage += status.CriticalDamage.Multiply(value);
        Accuracy += status.Accuracy.Multiply(value);
        Evasion += status.Evasion.Multiply(value);
        Mobility += status.Mobility.Multiply(value);
        return this;
    }
